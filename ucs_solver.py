...
def ucs_solve(board, max_moves):
    minHeap = []                  
    heapq.heappush(minHeap, (0, board, []))  
    explored = set()  

    while minHeap:
        cost, current_board, path = heapq.heappop(minHeap)
        # print(cost)
        ...
        for piece_symbol in piece_symbols:
            ...
            for new_state, target_x, target_y in possible_states:
                ...
                if new_state not in explored:
                    heapq.heappush(minHeap , (new_cost, new_state, new_path))
